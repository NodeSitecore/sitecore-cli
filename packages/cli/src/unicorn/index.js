...
module.exports = {
  ...
  getUnicornSharedSecretKey(options) {
    ...
    try {
      ...
      log.error(
        chalk.red(
          '[error] Unable to read your shared secret key in your Sitecore website. You must run `nsc` under your project root directory.'
        )
      );
      /* istanbul ignore next */
      return process.exit();
  ...
  getConfigurations(options) {
    ...
    const url = module.exports.getUrl(options);
    const secret = module.exports.getUnicornSharedSecretKey(options);
    const syncScript = `${path.join(__dirname, '/GetConfig.ps1')} -secret ${secret} -url ${url} -configUrl ${
      options.siteUrl
    }/${UNICORN_CONF}`;
    const child = runScript(syncScript);
