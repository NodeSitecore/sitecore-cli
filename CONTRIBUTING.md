...
First, thank you for considering contributing to NodeSitecore! It's people like you that make the open source community such a great community! 😊
...
```bash
$ git clone https://github.com/YOUR_USERNAME/sitecore-cli.git
```
...
And on your sitecore project, use `npm link`:

```bash
npm link ../path/to/sitecore-cli
```
...
> Note: This action works only on the NodeSitecore repository (not on your fork).

### Guidelines

- NodeSitecore follow the git flow to generate a release note. To write your commit message see https://docs.google.com/document/d/1QrDFcIiPjSLDn3EL15IJygNPiHORgU1_OOAqWjiDU5Y/edit
